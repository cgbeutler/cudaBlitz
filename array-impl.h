 ...
 *
 * - Added a memory allocation policy flag to the array
 *   - Added to each constructor and memory allocation call
 *
 ***************************************************************************/
...
public:
    ...
    explicit Array(_bz_ArrayExpr<T_expr> expr, managedMemoryPolicy managedMemory = notManagedMemory);
    ...
    Array(GeneralArrayStorage<N_rank> storage = GeneralArrayStorage<N_rank>(),
		managedMemoryPolicy managedMemory = notManagedMemory)
        : MemoryBlockReference<T_numtype>(managedMemory),
          storage_(storage)
    {
        length_ = 0;
        stride_ = 0;
    ...
    Array(T_numtype* restrict dataFirst, TinyVector<int, N_rank> shape,
        ...
        GeneralArrayStorage<N_rank> storage = GeneralArrayStorage<N_rank>(),
		managedMemoryPolicy memoryPolicy = notManagedMemory)
      : MemoryBlockReference<T_numtype>(product(shape), dataFirst,
          neverDeleteData, memoryPolicy),
        storage_(storage)
    {
        BZPRECONDITION(dataFirst != 0);
    ...
    {
        length_ = extent;
        setupStorage(N_rank - 1, managedMemory);
    }
    ...
    Array(const TinyVector<int, N_rank>& lbounds,
		...
		const managedMemoryPolicy memoryPolicy = notManagedMemory);
...
#ifdef BZ_NEW_EXPRESSION_TEMPLATES
        : MemoryBlockReference<T_numtype>(notManagedMemory),
    ...
    T_array                           copy() const;
    T_array                           copy(managedMemoryPolicy memoryPolicy) const;
	...
	// TODO: If it will be used, add the ability to change memoryPolicy on resize
	//       This will be much faster than having to call
	//       resize(...).copy(newMemoryPolicy), which does an extra allocation
...
public:
    ...
    T_numtype* getInitializationIterator() { return dataFirst(); }
...
protected:
    ...
    _bz_inline2 void setupStorage(int rank, managedMemoryPolicy managedMemory);
