 ...
 *
 * - Fixed copying and referencing to inherit memory policy from the other
 *       array unless a change is specified when copying
 *
 ***************************************************************************/
...
Array<P_numtype,N_rank>::Array(_bz_ArrayExpr<T_expr> expr, managedMemoryPolicy memoryPolicy)
	: MemoryBlockReference<P_numtype>(notManagedMemory)
{
...
Array<P_numtype,N_rank>::Array(const TinyVector<int, N_rank>& lbounds,
	...
	const managedMemoryPolicy memoryPolicy)
    : MemoryBlockReference<P_numtype>(memoryPolicy),
		storage_(storage)
{
    length_ = extent;
    storage_.setBase(lbounds);
    setupStorage(N_rank - 1, memoryPolicy);
}
 ...
 */
template<typename P_numtype, int N_rank>
_bz_inline2 void Array<P_numtype, N_rank>::setupStorage(int lastRankInitialized, managedMemoryPolicy memoryPolicy)
{
    ...
    if (numElem==0)
        MemoryBlockReference<P_numtype>::changeToNullBlock(memoryPolicy);
    else
        MemoryBlockReference<P_numtype>::newBlock(numElem, memoryPolicy);
...
Array<P_numtype, N_rank> Array<P_numtype, N_rank>::copy() const
{
	if (numElements())
	{
		Array<T_numtype, N_rank> z(length_, storage_, memoryPolicy());
		z = *this;
		return z;
	}
	else {
...
template<typename P_numtype, int N_rank>
Array<P_numtype, N_rank> Array<P_numtype, N_rank>::copy(managedMemoryPolicy memoryPolicy) const
{
	if (numElements())
	{
		Array<T_numtype, N_rank> z(length_, storage_, memoryPolicy);
	...
	{
		if (memoryPolicy == this->memoryPolicy())
		{
			// Null array-- don't bother allocating an empty block.
			return *this;
		}
		...
		{
			// We need to create a new empty one to get the proper memoryPolicy variable
			Array<T_numtype, N_rank> z(length_, storage_, memoryPolicy);
			// Null array-- no need to copy stuff over
			return z;
		}
	}
}
